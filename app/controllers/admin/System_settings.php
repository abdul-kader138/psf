...
class system_settings extends MY_Controller
{
    ...
    function permissions($id = NULL)
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $data = array(
                ...
                'sales_achievement-sales_officer_add' => $this->input->post('sales_achievement-sales_officer_add'),
    ...
    function add_zone()
    {
        ...
        if ($this->form_validation->run() == true && $this->settings_model->addZone($data)) {
            ...
            $this->data['error'] = validation_errors() ? validation_errors() : $this->session->flashdata('error');
            $this->data['zones'] = $this->settings_model->getParentZones();
            $this->data['modal_js'] = $this->site->modal_js();
    ...
    function edit_zone($id = NULL)
    {
        ...
        $pr_details = $this->settings_model->getZoneByID($id);
        if ($this->input->post('code') != $pr_details->code) {
            $this->form_validation->set_rules('code', lang("Code"), 'required|is_unique[zones.code]');
        }
        $this->form_validation->set_rules('name', lang("Name"), 'required|min_length[3]');
        $this->form_validation->set_rules('description', lang("description"), 'trim|required');
        $this->form_validation->set_rules('bu', lang("Business_Unit"), 'trim|required');
        ...
        if ($this->form_validation->run() == true && $this->settings_model->updateZone($id, $data)) {
            ...
            $this->data['error'] = validation_errors() ? validation_errors() : $this->session->flashdata('error');
            $this->data['zone'] = $this->settings_model->getZoneByID($id);
            $this->data['zones'] = $this->settings_model->getParentZones();
            $this->data['modal_js'] = $this->site->modal_js();
    ...
    function delete_zone($id = NULL)
    {
        ...
        if ($this->site->getSubZone($id)) {
            $this->sma->send_json(array('error' => 1, 'msg' => lang("Zone_has_subzone")));
        }
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (!empty($_POST['val'])) {
                ...
                if ($this->input->post('form_action') == 'export_excel') {
                    ...
                    $row = 2;
                    foreach ($_POST['val'] as $id) {
                        $sc = $this->settings_model->getZoneByID($id);
                        $parent_actegory = '';
                        if ($sc->parent_id) {
                            $pc = $this->settings_model->getZoneByID($sc->parent_id);
                            $parent_category = $pc->code;
                        }
                        $this->excel->getActiveSheet()->SetCellValue('A' . $row, $sc->code);
                        $this->excel->getActiveSheet()->SetCellValue('B' . $row, $sc->name);
