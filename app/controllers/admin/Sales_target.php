        ...
        if ($this->form_validation->run() == true) {
            ...
            if (isset($_FILES["userfile"])) {
                ...
                if (!$this->upload->do_upload()) {
                    $error = $this->upload->display_errors();
                    $this->session->set_flashdata('error', $error);
                    admin_redirect("sales_target/sales_officer_add");
                }
                ...
                $keys = array('user_code', 'target_quantity','dealer','no_of_visit');
                $final = array();
                foreach ($arrResult as $key => $value) {
                    $final[] = array_combine($keys, $value);
                }
                ...
                foreach ($final as $csv_pr) {
                    if (isset($csv_pr['user_code']) && isset($csv_pr['target_quantity'])) {
                        ...
                        $target_details = $this->sales_target_model->getUserByCode($csv_pr['user_code']);
                        if (!$target_details) {
...
//        }
        ...
        $meta = array('page_title' => lang('Sales_Officer_Target'), 'bc' => $bc);
        $this->page_construct('sales_target/sales_officer_target', $meta, $this->data);
    }
    ...
    function getSalesOfficer()
    {
        ...
        if ($get_permission['employees-bill_delete'] || $this->Owner || $this->Admin) $delete_link = "<a href='#' class='po' title='<b>" . lang("Delete") . "</b>' data-content=\"<p>"
            ...
            . lang('i_m_sure') . "</a> <button class='btn po-close'>" . lang('no') . "</button>\"  rel='popover'><i class=\"fa fa-trash-o\"></i> "
            . lang('Delete') . "</a>";
        $detail_link = anchor('admin/sales_target/sales_officer_target_details/$1', '<i class="fa fa-file-text-o"></i> ' . lang('Target_details'));
        $action = '<div class="text-center"><div class="btn-group text-left">'
            . '<button type="button" class="btn btn-default btn-xs btn-primary dropdown-toggle" data-toggle="dropdown">'
            . lang('actions') . ' <span class="caret"></span></button>
        <ul class="dropdown-menu pull-right" role="menu">
        ...
        $this->datatables
            ...
            ->group_by('sales_officer_target.year,sales_officer_target.month,categories.name,zones.name')
            ->edit_column('active', '$1__$2', 'active, id')
            ->add_column("Actions", $action, "id");
        echo $this->datatables->generate();
    }

    public function sales_officer_target_details($bill_id = null)
    {
        if (!$this->Owner && !$this->Admin) {
            $get_permission = $this->permission_details[0];
            if ((!$get_permission['employees-bill_index'])) {
        ...
        $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
        $info = $this->sales_target_model->getAllOfficersTargetDetails($bill_id);
        $this->data['id'] = $bill_id;
        $this->data['rows'] = $info;
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('Sales_Target')), array('link' => '#', 'page' => lang('View_Target_Details')));
        $meta = array('page_title' => lang('View_Target_Details'), 'bc' => $bc);
        $this->page_construct('sales_target/officer_target_details', $meta, $this->data);
    ...
    function delete_sales_officer_target($id = NULL)
    {
        ...
        if ($this->sales_target_model->deleteSalesOfficerTarget($id)) {
            if ($this->input->is_ajax_request()) {
                echo lang("Info_Deleted");
                die();
            }
            $this->session->set_flashdata('message', lang('Info_Deleted'));
            admin_redirect('sales_target/sales_officer_target');
        }
    }
