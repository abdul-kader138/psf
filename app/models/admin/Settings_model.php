...
//    a.kader
    ...
    {
        $q = $this->db->get_where('zones', array('code' => $code), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
    ...
    public function addZones($categories, $subcategories)
    {
        $result = false;
        if (!empty($categories)) {
            foreach ($categories as $category) {
        ...
        if (!empty($subcategories)) {
            foreach ($subcategories as $category) {
                if (is_int($category['parent_id'])) {
                    ...
                    if ($pcategory = $this->getZoneByCode($category['parent_id'])) {
                        $category['parent_id'] = $pcategory->id;
                        $this->db->insert("zones", $category);
                    }
                }
    ...
    {
        if ($this->db->delete("zones", array('id' => $id))) {
            return true;
        }
        return FALSE;
    }
