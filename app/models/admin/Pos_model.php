...
class Pos_model extends CI_Model
{
    ...
    public function addSale($data = array(), $items = array(), $payments = array(), $sid = NULL)
    {
        ...
        if ($this->db->insert('sales', $data)) {
            ...
            $repair_status=array(
                'sale_id'=>$sale_id,
                'repair_status'=>$data['type_detail'],
                'repair_note'=>$data['repair_note'],
                'created_by' => $this->session->userdata('user_id'),
                'created_date' => date("Y-m-d H:i:s")
            );
            if($data['type']=='Repair') $this->db->insert('repair_history', $repair_status);
    ...
    public function getTodayDebitSales()
    {
        $date = date('Y-m-d 00:00:00');
        $this->db->select('COUNT(' . $this->db->dbprefix('payments') . '.id) as total_cc_slips, SUM( COALESCE( grand_total, 0 ) ) AS total, SUM( COALESCE( amount, 0 ) ) AS paid', FALSE)
            ->join('sales', 'sales.id=payments.sale_id', 'left')
            ->where('payments.type', 'received')->where('payments.date >', $date)->where('paid_by', 'Debit_Card');
    ...
    public function getRegisterDebitSales($date, $user_id = NULL)
    {
        if (!$date) {
            $date = $this->session->userdata('register_open_time');
        }
