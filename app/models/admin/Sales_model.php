...
class Sales_model extends CI_Model
{
    ...
    public function topupGiftCard($data = array(), $card_data = NULL, $ca_data = Null)
    {
        $this->db->trans_strict(TRUE);
        $this->db->trans_start();
        $this->db->insert('gift_card_topups', $data);
    ...
    {
        $q = $this->db->get_where('repair_history', array('id' => $id), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
    ...
    public function updateRepairStatus($inv, $repair_status, $sale_status)
    {
        $this->db->trans_strict(TRUE);
        $this->db->trans_start();
        $this->db->update('sales', $sale_status, array('id' => $inv));
        $this->db->insert('repair_history', $repair_status);
        $this->db->trans_complete();
        if ($this->db->trans_status() === FALSE) return false;
        return true;
    }
    ...
    {
        $this->db->select('sales.id,sales.customer,sales.reference_no,companies.name as nam,repair_history.created_by,repair_history.created_date,repair_history.repair_note,repair_history.repair_status,users.username')
            ->join('repair_history', 'repair_history.sale_id=sales.id', 'left')
            ->join('companies', 'companies.id=sales.customer_id', 'left')
            ->join('users', 'users.id=repair_history.created_by', 'left');
    ...
    public function getStatusByID($id)
    {
        ...
        $this->db->order_by('sales.id', 'desc')->where('sales.id', $id);
        $q = $this->db->get();
        if ($q->num_rows() > 0) {
            return $q->row();
        }
